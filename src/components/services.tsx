...
export default async function Service({ services }: ServiceProps) {
  return (
        ...
        {services.map((service, index) => (
          <Link href={`/services/${service.slug}`} key={index}>
            <Card
              ...
              key={index}
            >
              <CardTitle className="py-4 underline text-center text-white text-2xl font-bold">
                {service.title}
              </CardTitle>
              <CardDescription className=" text-white mb-8 text-center">
                {service.info}
              </CardDescription>
            </Card>
          </Link>
